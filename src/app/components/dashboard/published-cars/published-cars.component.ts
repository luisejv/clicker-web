...
export class PublishedCarsComponent implements OnInit {
  ...
  changeBrand(e: any | string): void {
    ...
    if (this.carType?.value === '') {
      ...
      this.filteredCarros = this.backupFilteredCarros.filter((carro: AutoSemiNuevo) => {
  ...
  changeModel(e: any): void {
    ...
    if (this.carType?.value !== '') {
      this.carFilters.forEach((filter: Filter) => {
        if (filter.tipoCarroceria === this.carType?.value && filter.modelo === model) {
          resetType = false;
        }
      });
    }
  ...
  changeCarroceria(e: any): void {
    ...
    if (this.carBrand?.value !== '' && this.filteredBrands.indexOf(this.carBrand?.value) === -1) {
      ...
      this.filteredModels = this.carFilters
      .filter((elem: Filter) => {
        return elem.marca === this.carBrand?.value && elem.tipoCarroceria === type;
      })
      .map((elem) => elem.modelo)
      .filter((v, i, a) => a.indexOf(v) == i);
      setTimeout(() => {
  ...
  changeDepartamentos(e: any): void {
    ...
    this.filteredCarros = this.filteredCarros.filter(
      (carro: AutoSemiNuevo) => {
        ...
        this.carDepartamentos?.value.forEach(
          (departamento: string) => {
            johnathanPrieto = johnathanPrieto ||
